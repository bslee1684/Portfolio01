...
//	protected static final String[] BUFFER_KEY_ARR = new String[]{"beIPaddr", "beSensorDate", "beAvgTemp", "beAvgHumi", "beAvgCo2", "beAvgNh3", "beAvgDust"};
//	protected static final String[] SENSOR_KEY_ARR = new String[]{"siSensorDate", "siTemp", "siHumi", "siCo2", "siNh3", "siDust", "siWeight"};
	
	// id가 세팅된 인스턴스 맵
	public static ConcurrentHashMap<String, FarmInfo> list_map = new ConcurrentHashMap<String, FarmInfo>();
	protected static final int CHECK_UPDATE_COUNT = (int) (long)FileUtil.get_config("check_in_count");						//입추 기준 - 10개
	protected static final long CHECK_TIME_BACKUP = (long)FileUtil.get_config("check_time_backup");					//백업 판단 시간
	
	protected String farm_id = "";
	protected String dong_id = "";
	protected String ip_addr = "";
	
	protected boolean is_backup = false;
	protected boolean is_recalc = false;
	
	//하층부 저울 데이터가 들어온 경우에만 업데이트 하도록 하는 변수
	protected boolean updatable = false;
	
	protected String last_sensor_date = "";													//최종 데이터 수집 시간
	protected String first_update_date = "";													//데이터 최초 적재 시간 - in_check 가 1일 때 시간
	protected String backup_start_date = "";													//백업 시작시간
	
	protected Long last_update_stamp = DateUtil.get_inst().get_now_timestamp();				//최종 데이터 업데이트 시간		-- my_timer에 의해서 실행됨
	protected Long last_work_stamp = DateUtil.get_inst().get_now_timestamp();					
	
	protected int update_count = 0;
	
	protected HashMap<String, SensorUnit> sensor_unit_map = null;
	protected ExternSensor extern_sensor = null;
	protected LightSensor light_sensor = null;
	protected MultiFeedSensor multi_feed_sensor = null;
	
	protected List<Document> sensor_data_list = new ArrayList<Document>();
	
	protected HashBuffer history_buffer = new HashBuffer();
	
	protected FarmInfo(String farm, String dong){
		farm_id = farm;
		dong_id = dong;
	...
	public void delete_inst(){
		...
		FileUtil.write("CHECK => delete_inst() : " + farm_id + dong_id);
		
		for(SensorUnit unit : sensor_unit_map.values()) {
			unit.delete_inst();
		}
		
		if(extern_sensor != null) {
			extern_sensor.delete_inst();
		}
		
		if(light_sensor != null) {
			light_sensor.delete_inst();
		}
	...
	public MultiFeedSensor get_multi_feed_sensor() {
		if(multi_feed_sensor == null){
			multi_feed_sensor = MultiFeedSensor.get_inst(farm_id, dong_id, this);
		}
		return multi_feed_sensor;
	}
	
	public boolean has_multi_feed_sensor(){
		if(multi_feed_sensor == null){
			return false;
		}
		return true;
...
//	}
	...
	public void set_updatable(boolean updatable) {
		this.updatable = updatable;
	}
	
	public void work() {
		
		//FileUtil.write(sensor_data_list.toString());
		
		// 저울 데이터 없으면 동작 안함
		if(sensor_data_list.size() <= 0) {
//			if(farm_id.equals("KF0071")) {
//				FileUtil.write("ERROR => sensor_data_list is empty in : " + farm_id + " " + dong_id);
//			}
			return;
		}
	...
	public void run_mongo_update() throws MongoException, Exception{
		// sensorData 적재
...
//	protected void run_update_buffer() {			//쿼리문을 만들어 buffer_table_update_queue에 적재
//		
...
//	}
//	
//	protected void run_update_cell() {			//쿼리문을 만들어 set_iot_cell 업데이트
//		
//		// buffer_sensor_status 업데이트
//		
//		
	...
	public HashMap<String, String> get_buffer_data(){
		...
		for(Entry<String, SensorUnit> entry : sensor_unit_map.entrySet()){
			...
			try {
				...
				FileUtil.write("TEST => Exception : " + cell.get_doc().toJson());
			}
		}
...
//		history_buffer.put("cell_dust", avg_dust);
	...
	protected double get_sensor_avg(String key){
		double avg = 0.0;
		int cnt = 0;
		for(Entry<String, SensorUnit> entry : sensor_unit_map.entrySet()){
			SensorUnit cell = entry.getValue();
			...
			// 상층부 센서 계산 안함
			int cell_num = Integer.parseInt(cell.get_cell_id());
			if(cell_num >= 13) {
				continue;
			}
			...
			try {
				...
				double value = (double) cell.get_double(key);
				if(value > 0){
				...
				FileUtil.write("ERROR => ClassCastException : " + key + " -> "+ cell.get_doc().toJson());
