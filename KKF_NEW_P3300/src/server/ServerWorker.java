...
public abstract class ServerWorker implements Runnable {
	...
	//데이터 수신을 위한 변수 선언
	private byte[] input_bytes = new byte[1024];		//inputstream에서 들어온 바이트를 읽어서 저장함
	private int input_count = 0;						//inputstream에서 들어온 바이트 배열 길이
		...
		try {
			...
			while (dis != null && !interrupt) {
				...
				if (input_count == -1) {
					...
					receive_work(input_count, input_bytes);
				}
			}
	...
	// 패킷 수신 시 할 동작
	protected abstract void receive_work(int count, byte[] input);
			...
			if(len < 4) {		//data Length(4번째 바이트)까지 무조건 적재
				...
				if(goal == len + 1) {		//packetList에 적재된 길이가 올바른 패킷 길이와 같은 경우
					if (packet_maker.get(0) == byte_head		//시작비트
						...
						try {
							byte[] test = new byte[packet_maker.size()];
							
							int i = 0;
							for(byte b : packet_maker) {
								test[i++] = b;
							}
							FileUtil.write("ERROR => Byte Parsing Error in check_packet() " + id + " ---- " + FileUtil.byte_to_string(test));
						} catch (Exception e) {
							e.printStackTrace();
						}
						packet_maker.clear();
