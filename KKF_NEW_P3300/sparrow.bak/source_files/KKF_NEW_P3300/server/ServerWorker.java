...
public abstract class ServerWorker implements Runnable {
	...
	public void run() {
		try {
			...
			int tPort = 0;
			if(socket != null) {
				tIp = socket.getInetAddress() != null ? socket.getInetAddress().toString() : "";
				tPort = socket.getPort() <= 0 ? socket.getPort() : 0;
			}
			FileUtil.write("ALARM => " + "Socket IOException : " + tIp + ":" + tPort);
			stream_close();
	...
	// 패킷 수신 시 할 동작
	protected abstract void receive_work(byte[] packet);
